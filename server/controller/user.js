...
async function sendOtp(req, res) {
    ...
    try {
        ...
        return res
            .status(500)
            .json({ error: e, message: "DB interaction not working" });
    }
    if (tem.length == 0) {
    } else {
        return res
...
async function uploadProfilePhoto(req, res) {
    try {
        ...
        return res
            .status(500)
            .json({ message: "Error uploading profile photo", error });
    }
}
...
async function makeUser(req, res) {
    ...
    if (!body.name || !body.email || !body.password) {
        return res
            ...
            .json({
                success: false,
                message: "User creation failed due to missing fields",
            });
    }
    body.email = body.email.trimEnd().toLowerCase();
    ...
    if (!body.otp) {
        return res
            .status(400)
            .json({ success: false, message: "provide OTP for user creation" });
    }
    const type = await verifyOtp(body.email, body.otp);
    if (type == 2) {
        return res
            .status(400)
            .json({ success: false, message: "Incorrect OTP provided" });
    } else if (type == 3) {
        return res
            .status(400)
            .json({ success: false, message: "OTP you provided is expired" });
    } else if (type != 1) {
...
async function loginUser(req, res) {
    ...
    if (!User) {
        return res
            .status(400)
            .json({ success: false, message: "email not present in data base" });
    }
...
async function forgotPasswordOtp(req, res) {
    ...
    try {
        ...
        return res
            .status(500)
            .json({ message: "database cannot be reached", error: error.message });
    }
    if (temp == null) {
        return res
            .status(400)
...
async function forgotPassword(req, res) {
    ...
    if (!body.email) {
        return res
            .status(500)
            .json({ success: false, message: "React error email not provided" });
    }
    body.email = body.email.trimEnd().toLowerCase();
    if (!body.password) {
        return res
    ...
    try {
        ...
        return res
            .status(500)
            .json({ message: "unable to update password", error: e.message });
    }
